...
import { ShopCategory, shopCategories } from '@/constants/shop';
...
export function ShopCard({ product, lang, translations }: ShopCardProps) {
  const isRTL = lang === 'ar';
  const localizedName = (isRTL ? product.nameAr : product.name) || product.name;
  const localizedDescription = (isRTL ? product.descriptionAr : product.description) || product.description;
